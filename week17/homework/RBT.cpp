...
public:
    ...
    //Dont fully understand T and T1  
    template <class T1>
...
public:
    ...
    void balanceInsertion(RBTNode<T> *point);
...
//This function might be causing my segmentation fault
template <class T>
void RBT<T>::balanceInsertion(RBTNode<T> *point) {


   while ((point != root) && (getColor(point->parent) == RED &&  (getColor(point) != BLACK))) {
